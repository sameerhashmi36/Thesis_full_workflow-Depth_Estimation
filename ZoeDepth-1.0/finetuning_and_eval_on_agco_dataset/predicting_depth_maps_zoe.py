...
ZoeDepth prediction + visualization for AGCO (similar to monodepth2 predictor).

Modes:
1) Index mode: --idx N
   - Builds an "ALL" dataset across all common bags (including test bags).
   - Uses paired rectified + depth_z16 to ensure GT exists.
...
3) Bag clip mode: --bag <bag_name>
   ...
   - GT found by direct filename transform (no glob per frame).

Optional:
- --use-smooth-gt: apply Telea-limited smoothing to GT
- --median-scaling: scale pred by median(gt)/median(pred) on valid GT pixels (per-frame / per-image)
...
# -------------------------
# Depth visualization (OpenCV 3.3.1)
# -------------------------
...
def depth_to_gray_near_white(depth_m: np.ndarray, vmin: float, vmax: float) -> np.ndarray:
    ...
    norm = (d - vmin) / (vmax - vmin + 1e-8)  # near->0, far->1
    g = (np.clip(norm * 255.0, 0, 255)).astype(np.uint8)
    g = 255 - g  # invert: near white
    return g

def colorize_depth_near_bright(depth_m: np.ndarray, vmin: float, vmax: float) -> np.ndarray:
    """
    Near=bright, far=dark, OpenCV 3.3.1 friendly:
    - normalize depth -> 0..255, invert, then COLORMAP_HOT
    """
    d = depth_m.astype(np.float32).copy()
    d[~np.isfinite(d)] = vmin
    d = np.clip(d, vmin, vmax)
...
def load_state_dict_strict(model: nn.Module, ckpt_path: str, device: torch.device):
    """
    Minimal strict loader:
    - Accepts a .pth which may contain state_dict directly or nested dict.
    - Loads with strict=True.
    """
    state = torch.load(ckpt_path, map_location=device)
    if isinstance(state, dict) and "state_dict" in state and isinstance(state["state_dict"], dict):
        state = state["state_dict"]
    # Some checkpoints may have 'model' key etc.
    if isinstance(state, dict) and "model" in state and isinstance(state["model"], dict):
        state = state["model"]
    model.load_state_dict(state, strict=True)

def build_zoe_model(repo_root: Path, model_name: str, base_ckpt: Path, finetuned_weights: Path, device: torch.device) -> nn.Module:
    if str(repo_root) not in sys.path:
        sys.path.append(str(repo_root))

    zoe = torch.hub.load(str(repo_root), model_name, source="local", pretrained=False)

    # base ckpt strict
    load_state_dict_strict(zoe, str(base_ckpt), device=device)

    # finetuned strict (state_dict)
    ft_state = torch.load(str(finetuned_weights), map_location=device)
    if isinstance(ft_state, dict) and "state_dict" in ft_state and isinstance(ft_state["state_dict"], dict):
        ft_state = ft_state["state_dict"]
    zoe.load_state_dict(ft_state, strict=True)

    zoe.to(device).eval()
    return zoe

@torch.no_grad()
def predict_depth_m_zoe(zoe: nn.Module, rgb_1chw: torch.Tensor, device: torch.device, out_hw: Tuple[int, int]) -> np.ndarray:
    rgb_1chw = rgb_1chw.to(device)
    pred = forward_zoe_metric(zoe, rgb_1chw)  # [1,1,h,w]
...
class AGCOAllDepthDataset(Dataset):
    """
    Single list of (rgb_u8, gt_depth_m, img_path, depth_path) across ALL common bags.
    """
...
# -------------------------
# Median scaling helper (for visuals)
# -------------------------
def apply_median_scaling_if_enabled(pr_m: np.ndarray, gt_m: np.ndarray, do_scale: bool) -> np.ndarray:
    if not do_scale:
        return pr_m
    m = (gt_m > MIN_DEPTH) & (gt_m < MAX_DEPTH) & np.isfinite(gt_m) & (pr_m > 0) & np.isfinite(pr_m)
    if not m.any():
        return pr_m
    gt_flat = gt_m[m]
    pr_flat = pr_m[m]
    scale = float(np.median(gt_flat) / (np.median(pr_flat) + 1e-8))
    pr_scaled = np.clip(pr_m * scale, 0.0, float(MAX_DEPTH))
...
# modes
# -------------------------
def run_index_mode(args, zoe, device, model_tag: str):
    ...
    rgb_u8, gt_sparse, img_path, depth_path = ds[idx]
    ...
    pr_m = predict_depth_m_zoe(zoe, rgb_u8_to_tensor_1chw(rgb_u8), device, out_hw=(args.img_height, args.img_width))
...
def run_folder_mode(args, zoe, device, model_tag: str):
...
def run_bag_clip_mode(args, zoe, device, model_tag: str):
    ...
    pr0 = predict_depth_m_zoe(zoe, rgb_u8_to_tensor_1chw(first_rgb), device, out_hw=(args.img_height, args.img_width))
    pr0 = apply_median_scaling_if_enabled(pr0, gt0, args.median_scaling)

    # vmin/vmax from gt if possible, else pred
...
def parse_args():
    ...
    p.add_argument("--out-dir", type=str, default="image_prediction/out_vis",
    ...
    p.add_argument("--base-ckpt", type=str, default="checkpoints/ZoeD_M12_K.pt",
                   help="pretrained ZoeDepth checkpoint path (inside repo-root if relative)")
    p.add_argument("--finetuned-weights", type=str, default="models_finetuned_on_agco_smooth/ZoeD_K_train80_val80_smooth/best.pth",
                   help="finetuned state_dict .pth (strict=True)")

    p.add_argument("--img-width", type=int, default=ZOE_W)
    p.add_argument("--img-height", type=int, default=ZOE_H)
    ...
    # visuals
    p.add_argument("--median-scaling", action="store_true", help="scale pred by median(gt)/median(pred) on valid GT pixels")
...
def main():
    ...
    # model tag for output folder
    model_tag = f"{args.model}__{finetuned.parent.name}__{finetuned.stem}"

    # cache
    args._gt_cache = LRUCache(max_items=int(args.gt_cache)) if int(args.gt_cache) > 0 else None

    zoe = build_zoe_model(repo_root, args.model, base_ckpt, finetuned, device)
    print("[FT] strict=True load OK")

    # Priority: bag clip > index > folder
    if args.bag.strip():
        run_bag_clip_mode(args, zoe, device, model_tag)
        return
    if int(args.idx) >= 0:
        run_index_mode(args, zoe, device, model_tag)
        return
    run_folder_mode(args, zoe, device, model_tag)
